...
Client * Client::Create(const string &userName, const string &password, const string &addr)
{
...
int Client::GetResponse()
{
	while (true)
	{
		...
		if (ret > 0)
		{
			...
			{
				PUTS_LOCK;
				cout << "一位名叫" << data.data.chatRequest.userName << "的好友请求和你聊天，同意(y)还是拒绝(n)？"; 
				PUTS_UNLOCK;
